...
class Tools():
    ...
    def seq_2_pos(self, seq):
        ...
        :param seq: shape of (batch_size, seq_max_len)
        :return: add position code for each element in seq.
        """
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, max_seq_len, d_model, pad_idx):
        ...
        # pos表示第pos个位置变量(长度为d_model)。i表示该位置变量中某个元素的取值, sin(w_i * t)/cos(w_i * t) 
        pos_enc = np.array([
            [pos / np.power(10000, 2.0 * (i // 2) / d_model) for i in range(d_model)]
            for pos in range(max_seq_len)])  # 构建位置编码表，shape(max_seq_len, d_model)

        pos_enc[:, 0::2] = np.sin(pos_enc[:, 0::2])  # sin计算偶数位置
        pos_enc[:, 1::2] = np.cos(pos_enc[:, 1::2])  # cos计算奇数位置

        # 序列中的padding tokens不携带实际信息，相应embedding也同样设置。
        pos_enc[pad_idx] = 0.  
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        # 屏蔽序列之后的元素，让其注意力权重变为零。softmax生成总值为1的序列，因此需要先进行mask，确保权重一致。
        attn = attn.masked_fill(mask, -np.inf)  # 屏蔽掉序列补齐的位置 (B*h)*L*L, np.exp(-np.inf) == 0

        attn = self.softmax(attn)

        # 将随机性作用在注意力权重分布上，减少引入值V的信息；
        # 每步都增强随机性，dropout的随机性“和输入数据息息相关”
...
class PositionWiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        # 将学习特征的复杂映射H(x)，修改为学习其两者之间的差异H(x) - x。
...
class Mask():
    ...
    def padding_mask(self, seq_k, seq_q):
        ...
        pad_mask = seq_k.eq(param.pad)  # 通过比较是否相等，产生pad_mask B*T
    ...
    def no_padding_mask(self, seq):
        ...
        :return: B*T*T
        """
        return seq.ne(param.pad).type(torch.float).unsqueeze(-1)    # 比较是否不等
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_in, pad_mask, no_pad_mask):
        ...
        # pad用于补齐短序列，no_pad_mask逐元素乘法再次消除 pad位置的隐状态值
